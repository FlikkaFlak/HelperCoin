...
uint256 hashGenesisBlock("0xa8246bc1b1e681ed80cbf09a7d9f99bccfc1a7283f50ddefc435335da368fc90");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(537261531); // HelperCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HelperCoin Signed Message:\n";
...
{
    // HelperCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 7000000000;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 560000); // HelperCoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HelperCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HelperCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // HelperCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1fb4512c050384fd8340fc59c3d836df3d1ee37fd16239e0084330f8c27ebd98");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Helper is nigh";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 7000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("8c107af01bd4e09802901593bc2c207392eb289173dcb1a7ceda5fe7c7e9e162807ee8bfd368652b9f5337a84911e1e8bf0a679c0668a18ded6e3b22bc78c663bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1491081208;
        block.nBits    = 537261531;
        block.nNonce   = 3389473856;

        if (fTestNet)
        {
            block.nTime    = 1491081208;
            block.nNonce   = 842089561;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x86f62d4b1c81dde4f7db560ba650ab06fa93fadf9b35d224957364671a1ffd9f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // HelperCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HelperCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HelperCoinMiner : ProcessBlock, block not accepted");
    }
